...
> אפליקציית ווב מלאה: מפת חום בארבע רמות, מפת חום מנהלה, לוח בקרה, סטטוס
> תחנות, התראות וניהול נתונים. מותאמת מלאה לנייד.

![מפת חום גיוס — תצוגה מקדימה](docs/hero.svg)

> ### ⚠️ הבהרת נתונים
> **כל מספר במאגר הזה מוגרל.** אחוזי האיוש, התקן מול המאויש, פילוח התפקידים,
> הדמוגרפיה, המועמדים בהליך, זמני הנסיעה ו-12,583 שורות המשרה — כולם מהתפלגות
...
| **הגדרות** | שלוש לשוניות: ניהול נתונים (7 מערכי נתונים), ספי צבע ומרחק, וטעינת אקסלים עם Data Guard. בתוכה גם **מבנה הנתונים · בדיקה עצמית** — כל טבלה במאגר, המספר שבה מול המספר שהמסך מציג, והפער ביניהם אם קיים. |
...
```
.
├── index.html                    # מבנה המסכים (6 מסכים)
...
├── data/                         # תשובות ה-API, קפואות (116 קבצים)
